...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const { data: profile } = useById(session?.user?.id || "") as {
    data: Profile | null;
  };
  ...
  const updateUserStatus = async (userId: string, status: UserStatus) => {
    ...
    try {
      ...
      toast({
        title: "Eroare",
        description:
          error.message || "A apărut o eroare la actualizarea statusului.",
        variant: "destructive",
      });
      return false;
    } finally {
  ...
  const inviteUser = async (email: string, role: UserRole) => {
    ...
    try {
      console.log(
        "Calling server-side inviteUserAction for",
        email,
        "with role",
        role
      );
      ...
      if (!result.success) {
        toast({
          title: "Eroare",
          description:
            result.error || "A apărut o eroare la invitarea utilizatorului.",
          variant: "destructive",
        });
        return false;
      }
      ...
      toast({
        title: "Eroare",
        description:
          error.message || "A apărut o eroare la invitarea utilizatorului.",
        variant: "destructive",
      });
      return false;
    } finally {
  ...
  const removeInvitedUser = async (userId: string) => {
    ...
    try {
      ...
      toast({
        title: "Eroare",
        description:
          error.message || "A apărut o eroare la eliminarea utilizatorului.",
        variant: "destructive",
      });
      return false;
    } finally {
  ...
  const resendInvitation = async (email: string) => {
    ...
    try {
      ...
      toast({
        title: "Eroare",
        description:
          error.message || "A apărut o eroare la retrimiterea invitației.",
        variant: "destructive",
      });
      return false;
    } finally {
  ...
  const completeProfile = async (data: ProfileUpdate) => {
    ...
    try {
      ...
      toast({
        title: "Eroare",
        description:
          error.message || "A apărut o eroare la actualizarea profilului.",
        variant: "destructive",
      });
      return false;
    } finally {
...
export function isAdmin(user: User | null, profile?: Profile | null) {
  if (!user) return false;
  // Verifică mai întâi în profile (sursă de adevăr), apoi fallback la user_metadata
  if (profile) {
